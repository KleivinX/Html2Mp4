...
Frames stream straight into FFmpeg as they're captured, so there is no temp
directory full of PNGs. Rendering uses **the Chromium that Electron already ships**: no
...
|---|---|---|
| **Mac** with Apple Silicon (M1 to M4) | `html2mp4-x.x.x-arm64.dmg` | ~127 MB |
